...
class CommentController extends AbstractController
{
    ...
    public function deleteC($crt)
    {
        ...
        $cmt = $this->getDoctrine()->getRepository(Commented::class)->findByContent($crt);
    ...
    public function editC(Request $request,$id,$crt): Response
    {
        ...
        $formE = $this->createForm(CommentType::class, $cmt[0]);
    ...
    /**
     * @Route ("/comm/{id}",name="deletecomment")
     */
    public function delete($id)
    {
        $em=$this->getDoctrine()->getManager();
        $cmt = $this->getDoctrine()->getRepository(Commented::class)->findByContent($id);

        $em->remove($cmt[0]);
        $em->flush();
        return $this->redirectToRoute('comm',['id'=>$id]);
     ...
     */
    public function comm(): Response
    {
        $posts = $this->getDoctrine()
            ->getRepository(Commented::class)
            ->findAll();
        return $this->render('BackOffice/CommentsDashboard.html.twig',['posts'=>$posts]);
    }
}
